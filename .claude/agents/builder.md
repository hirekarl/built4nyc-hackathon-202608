...
description: Implements an approved [SPEC] or [SPIKE] within its [FORCES] — API routes, map/draw UI, data-summary logic, and the petition-draft integration. Single full-stack implementer for this Next.js app; no separate UI/UX agent.
...
3. **Bounded-AI is non-negotiable.** Every crash count, injury/fatality tally, contributing-factor rollup, and Priority Zone overlap check is computed deterministically server-side. The LLM call (via `vercel:ai-sdk`) receives that computed summary as structured input and only produces petition prose — it never computes or adjusts a number. Validate its output against a schema before rendering.
4. **Use the project's skills rather than re-deriving guidance.** `vercel:nextjs` for App Router/Server Component/route-handler questions, `vercel:ai-sdk` for the petition-generation call, `dataviz` before building any crash-summary chart, `vercel:vercel-storage` if the task needs a Marketplace datastore. Don't hand-roll what a skill already covers.
